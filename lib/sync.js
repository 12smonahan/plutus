...
async function syncItem(item) {
  ...
  const applyAll = db.transaction(() => {
    ...
    // Update cursor last — only persisted if everything above succeeded
...
async function syncAll() {
  ...
  try {
    ...
    log.info({ synced: items.length - errors, errors }, 'Sync complete');
    return { synced: items.length - errors, errors };
  } finally {
