...
const db = new Database(dbPath, { verbose: process.env.DB_VERBOSE ? (msg) => log.debug({ sql: msg }, 'query') : undefined });
...
// Add status/error columns to plaid_items (safe to run repeatedly)
const itemCols = db.prepare("PRAGMA table_info(plaid_items)").all().map(c => c.name);
if (!itemCols.includes('status')) {
  db.exec("ALTER TABLE plaid_items ADD COLUMN status TEXT DEFAULT 'good'");
}
if (!itemCols.includes('error_code')) {
  db.exec("ALTER TABLE plaid_items ADD COLUMN error_code TEXT");
}
if (!itemCols.includes('error_message')) {
